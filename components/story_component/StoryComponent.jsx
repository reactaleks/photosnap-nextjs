  ...
  const componentList = storyData.map((item, index) => {
    return <div className="relative w-full md:grid md:grid-cols-12 md:h-[650px] " key={index}>
      <Image
        ...
        alt=""
        className={`object-cover w-screen md:h-full xl:h-[650px] md:col-span-4 ${index % 2 == 0 ? 'md:col-start-9 xl:col-start-6' : 'md:col-start-1 '} xl:col-span-7`}
      />
      <div className={`h-[419px] md:h-full  ${index == 0 ? 'bg-black text-white' : 'bg-white text-black'} ${index % 2 == 0 ? 'md:col-start-1 md:row-start-1' : 'md:col-start-5'} md:col-span-8 xl:col-span-5  flex flex-col items-cener justify-center relative`}>
